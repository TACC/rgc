...
def test_getPrograms(caplog):
	...
	assert ss.getPrograms(url, block=True) == list(programs[url] - {"time"})
	ss.block_set = {"ls"}
...
def test_findCommon(caplog):
	...
	ss.findCommon(p=51)
	assert ss.block_set == programs[urls[0]] & programs[urls[1]] | {'time'}
	ss.permit_set = {'ls'}
	ss.findCommon(p=51)
	assert ss.block_set == programs[urls[0]] & programs[urls[1]] - {'ls'} | {'time'}
...
def test_findCommon_baseline(caplog):
	...
	assert ss.block_set == {'1','2','3','time'}
